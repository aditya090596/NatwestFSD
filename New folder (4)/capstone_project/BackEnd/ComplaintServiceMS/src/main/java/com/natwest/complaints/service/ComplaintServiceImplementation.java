...
public class ComplaintServiceImplementation implements ComplaintServiceInterface{
    ...
    public Complaint registercomplaint(Complaint compObj) throws ServerDown {
        ...
        compObj.setRaisedBy(compObj.getRaisedBy());
        compObj.setRaisedAt(new Date(System.currentTimeMillis()));
        compObj.setCompID(uuid);

       Complaint newcom= this.compRepo.save(compObj);
        return newcom; // This should add to the store to be displayed on customer's dashboard.
    }
